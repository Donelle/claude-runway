...
Same role as Track D's savings estimate — a cheap diagnostic number, not a pass/fail result. Unlike Track D's estimate, though, nothing currently instruments `remember`/`recall` call frequency automatically — this ratio has to be derived manually from session transcripts today (same approach as Step A5), not read off a live counter; issue #179 tracks adding real instrumentation for it. A ratio heavily skewed toward `remember` suggests memories aren't being retrieved when relevant, but doesn't distinguish "nothing to recall yet" from "retrieval isn't working," so it shouldn't be reported as if it were Step E1-E3's result.
