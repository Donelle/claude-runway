...
- Changing `EMBEDDING_MODEL` after a collection has data requires manually dropping the collection (directly in Qdrant) and re-indexing from scratch — old and new vectors aren't compatible, and `index_repo --reset` can no longer do this rebuild itself (see the memory-bank-related bullet below for why).
...
- **The shared `memory-bank` collection locks in whichever `EMBEDDING_MODEL` created it first** (issue #175) — every project's memory-bank server writes into the SAME collection (unlike `codebase-indexer`'s per-project one), and Qdrant's named-vector schema is fixed at collection-creation time. A project configured with a different `EMBEDDING_MODEL` gets a clear, actionable mismatch error rather than silently-wrong results, but every project sharing this collection needs to agree on this one value.
- **`index_repo(reset=True)` no longer rebuilds a collection's vector schema at all** (issue #175) — it always does a filtered delete (preserving any `metadata.source == "memory-bank"` points) instead of a real `delete_collection`, for every collection, unconditionally. An earlier version only did this when the target already held memory-bank points (falling back to a fast full `delete_collection` otherwise) — but that count-then-act check and the delete itself were separate Qdrant requests with no transaction between them, so a concurrent `remember()` landing in the gap could still be destroyed by a "zero-count" full delete. Removing the count-based branch entirely removes that race instead of narrowing it. The practical cost: the one documented case just above (an `EMBEDDING_MODEL` change requiring a full schema rebuild) now requires manually dropping the collection outside this tool before re-running `index_repo` — `reset=True` alone can no longer do it.
